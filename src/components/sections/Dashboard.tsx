...
import { CheckCircle2, TrendingUp, Flame, Zap, MessageSquare, ChevronRight } from 'lucide-react';
  ...
  return (
    <div className="space-y-4 pb-2">
      ...
      <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.05 }}
        className="glass rounded-2xl p-5 flex items-center gap-5" style={{ background: 'rgba(59,130,246,0.06)', border: '1px solid rgba(59,130,246,0.15)' }}>
        ...
        <div className="flex-1">
          ...
          <p className="text-3xl font-bold text-white">{state.momentumScore}<span className="text-gray-500 text-base">/1000</span></p>
          <p className="text-xs text-gray-400 mt-1">Complete habits & goals to increase</p>
          <div className="mt-3">
      ...
      <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.08 }}
        ...
        <div className="flex items-center justify-between mb-4">
          <h3 className="font-semibold text-white text-sm">Today's Top Priorities</h3>
        ...
        <div className="space-y-3">
          {pendingHabits.slice(0, 3).map((habit, i) => (
            <motion.div key={habit.id} whileTap={{ scale: 0.98 }}
              ...
              <span className="flex-1 text-sm text-gray-300">{habit.name}</span>
              <span className="text-xs text-gray-600">#{i + 1}</span>
            </motion.div>
          ))}
          {pendingHabits.length === 0 && (
            <p className="text-sm text-emerald-400 font-medium">All habits completed! Outstanding work.</p>
          )}
        </div>
      </motion.div>
      ...
      {topGoal && (
        <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1 }}
          className="glass rounded-2xl p-5 cursor-pointer" style={{ background: 'rgba(255,255,255,0.03)' }}
          ...
          <p className="text-xs text-gray-500 mb-3">
            {topGoal.microGoals.filter(m => m.status === 'completed').length}/{topGoal.microGoals.length} milestones · Due {new Date(topGoal.deadline).toLocaleDateString('en-US', { month: 'short', day: 'numeric' })}
          </p>
          ...
          {state.habits.map(h => (
            <div key={h.id} className="flex flex-col items-center gap-1.5 shrink-0">
              <div className="w-11 h-11 rounded-2xl flex items-center justify-center text-xl"
                style={{ background: h.completedToday ? `${h.color}22` : 'rgba(255,255,255,0.05)', border: `1px solid ${h.completedToday ? h.color + '44' : 'transparent'}` }}>
                {h.icon}
              </div>
              <span className="text-xs font-bold" style={{ color: h.completedToday ? '#10b981' : '#fff' }}>
                {h.streak}d
              </span>
            </div>
          ))}
      ...
      <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.14 }}
        className="glass rounded-2xl p-5" style={{ background: checkInDone ? 'rgba(16,185,129,0.06)' : 'rgba(59,130,246,0.06)', border: `1px solid ${checkInDone ? 'rgba(16,185,129,0.2)' : 'rgba(59,130,246,0.15)'}` }}>
        <div className="flex items-center justify-between">
          <div>
            <h3 className="font-semibold text-white text-sm">{checkInDone ? 'Check-In Complete' : 'Daily Check-In'}</h3>
            <p className="text-xs text-gray-400 mt-0.5">{checkInDone ? '+25 Momentum added' : 'Start your day strong +25 pts'}</p>
          </div>
          <Button onClick={handleCheckIn} disabled={checkInDone} variant={checkInDone ? 'secondary' : 'primary'} size="sm">
        ...
        <AnimatePresence>
          {winSaved && (
            <motion.p initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
              className="text-xs text-emerald-400 mt-1.5">Win saved!</motion.p>
          )}
        </AnimatePresence>
      </motion.div>
      ...
      <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.18 }}
        className="glass rounded-2xl p-5 text-center" style={{ background: 'rgba(139,92,246,0.05)', border: '1px solid rgba(139,92,246,0.1)' }}>
